...
**Live:** https://world-cup-monitor.pages.dev
...
- ⏱️ Live match clock above the score: `NOT STARTED`, ticking `MM:SS`, `45:00 (+x)` / `90:00 (+x)`
  stoppage, `HALF TIME`, `HYDRATION BREAK`, `FULL TIME`.
...
- 🧮 Per-team summary under the score: yellow/red cards, shots, shots on target, subs left.
- 🔔 Goal alerts across all live matches: screen flash + optional beep/desktop notification
  (toggle in the toolbar), and a flip-board score animation when the score changes.
- 📈 Win-probability bar + match info (referee, attendance, weather) below the score.
- 🧑‍🤝‍🧑 Lineups: a vertical **formation pitch** (home top / away bottom) plus the full list,
  each player shown with their photo and annotated with goals ⚽, assists 🅰, cards 🟨🟥 and sub ⇄.
- 📲 Installable **PWA** (add to home screen, offline shell) + deep links (#matchId) so a refresh/share keeps the selected match.
- ⭐ Favorite teams (auto-select, highlighted) and a next-match countdown; adaptive polling (1s live / 5s idle, paused when the tab is hidden).
- 🏆 Knockout **bracket** (from openfootball) once the knockout stage starts.
- 🌐 Built-in Google Translate picker (top-right) — manual, no auto-translate; dynamic
  numbers/names are marked non-translatable to avoid flicker when switching language.
- 🏅 Field-by-field merge across sources; authority order FIFA > ESPN > openfootball > mock
  (the highest available source wins per field).
...
- ⏱️ 比分上方的比赛计时:`NOT STARTED`、跳秒 `MM:SS`、`45:00 (+x)`/`90:00 (+x)` 补时、
  `HALF TIME`、`HYDRATION BREAK`、`FULL TIME`。
...
- 🧮 比分下方的两队汇总:黄牌/红牌数、射门、射正、剩余换人次数。
- 🔔 全场进球提醒:任意 live 比赛进球时整屏闪 + 可选哔声/桌面通知(工具栏开关);
  比分变动用翻页牌(flip board)动画。
- 📈 比分下方的胜率预测条 + 比赛信息(裁判、上座、天气)。
- 🧑‍🤝‍🧑 Lineups:竖向**球场阵型图**(主队上 / 客队下)+ 完整名单,每名球员标注进球 ⚽、
  助攻 🅰、红黄牌 🟨🟥、换人 ⇄,并显示球员照片。
- 📲 可安装 **PWA**(添加到主屏幕、离线壳)+ 深链接(#matchId),刷新/分享保留所选比赛。
- ⭐ 收藏球队(自动选中、高亮)与下一场开球倒计时;自适应轮询(直播 1s / 空闲 5s,后台暂停)。
- 🏆 淘汰赛**对阵图**(来自 openfootball),淘汰赛阶段开始后显示。
- 🌐 内置 Google 翻译选择框(右上角)——手动、不自动翻译;动态数字/姓名标记为不翻译,
  避免切换语言时闪烁。
- 🏅 多源逐字段合并;权威顺序 FIFA > ESPN > openfootball > mock(每个字段取可用的最高优先级源)。
